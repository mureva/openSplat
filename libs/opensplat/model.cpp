...
void Model::savePlyText(const std::string &filename, int step){
	...
	float *m = static_cast<float*>( meansCpu.data_ptr() );
	float *fd = static_cast<float*>( featuresDcCpu.data_ptr() );
	float *fr = static_cast<float*>( featuresRestCpu.data_ptr() );
	float *op = static_cast<float*>( opacitiesCpu.data_ptr() );
	float *s = static_cast<float*>( scalesCpu.data_ptr() );
	float *q = static_cast<float*>( quatsCpu.data_ptr() );
    for (size_t i = 0; i < numPoints; i++) 
	{
		o << *m++ << " " << *m++ << " " << *m++ << "      ";
		o << *s++ << " " << *s++ << " " << *s++ << "      ";
...
{
	// I expect in to be (n,3) and on the cpu
    if( in.sizes().size() != 2 )
		throw std::runtime_error(" in needs to be 2D " );
	
	// make sure we have contiguous float tensor
	torch::Tensor cft = in.contiguous().to( torch::kFloat );

	// wrap an Eigen Map around the data.
	Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> M( cft.data_ptr<float>(), cft.sizes()[0], cft.sizes()[1]);
	Eigen::MatrixXf V = M; // hoped to avoid the copy but... meh

	// now we should be able to do the nanoflann stuff very easily.
	typedef nanoflann::KDTreeEigenMatrixAdaptor< Eigen::MatrixXf >  kd_tree_t;
    kd_tree_t tree( 3, V, 10 );
	...
	for( unsigned c = 0; c < V.rows(); ++c )
	{
		query[0] = V(c,0); query[1] = V(c,1); query[2] = V(c,2);
		tree.index->knnSearch( &query[0], count, indices.data(), sqr_dists.data() );
		float sum = 0.0;
	...
	return torch::from_blob( &buf[0], {V.rows(), 1 } );
}
